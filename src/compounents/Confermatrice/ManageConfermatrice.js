...
class ManageConfermatrice extends Component {
    constructor() {
        super()
        this.state = {
            first_name: "",
                ...
                if (!findresponse.status === 200 || findresponse.msg === 'Not enough segments') {
                    ...
                    if (!findresponse.user) {
                        ...
                        if (findresponse.user.role === "Livreur") {
                            ...
                            const result = []
                            fetch('http://151.80.56.201:5000/Dashboard/ManageConfermatrice', {
                                method: 'get', headers: {
                                    Accept: 'application/json',
                                    'Content-Type': 'application/json',
                                ...
                                .then((Response) => Response.json())
                                .then((findresponse) => {
                                    findresponse.Confermatricelist.map((row, i) =>
                                        result.push([row.id, row.first_name, row.last_name, row.phone, <><Link to={"/EditConfermatrice/" + row.id}><EditOutlined /></Link> <Button onClick={() => this.DeleteDeliver(row.id)} icon={<DeleteOutlined />}></Button>
                                        </>]))
                                    this.setState({
                                        DelivrerList: result
                                    })
    ...
    DeleteConfermatrice = (id) => {
        const formData = new FormData();
        formData.append("id", id)
        axios({
            url: `http://151.80.56.201:5000/Dashboard/ManageConfermatrice`,
            method: 'POST',
            data: formData,
            headers: {
...
export default ManageConfermatrice