...
class Delivred extends Component {
    constructor() {
        super()
        this.state = {
            first_name: "",
    ...
    componentDidMount() {
        ...
        let result = []
        fetch('http://151.80.56.201:5000/Dashboard/Delivred', {
            method: 'get', headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
                'Authorization': "Bearer " + token,
    ...
    Submit(i,e,livreur,benifice,details){
        const token = localStorage.getItem('usertoken');

        const formData = new FormData();
        formData.append("delivryPrice", this.state.values[i])
        formData.append("TotaleLivreur",livreur )
        formData.append("Benifice",benifice )
        formData.append("Details",JSON.stringify(details) )
        axios({
            url: "http://151.80.56.201:5000/Dashboard/Caisse",
            method: 'POST',
            data: formData,
            headers: {
                'Content-Type': 'multipart/form-data',
                        ...
                        <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
                            {
                                this.state.AllData !== [] ?
                                    this.state.AllData.map((item, key) => (
                                        <>
                                            <MUIDataTable
                                                title={item.NameLivreur + " " + item.LastNameLivreur}
                                                columns={["ID", "Nom", "Prénom", "Num Tél", "Totale", "Date"]}
                                                data={item.laivrason}
                                                options={{
                                                    filterType: "checkbox",
                                                    print: true
                                            ...
                                            <div className="row">
                                                <div className="col-3">
                                            <Text><b style={{color:"red"}}>Cout de Laivraison</b> <b style={{color:"blue"}}>Nombre de Laivraison:{item.nbc}</b></Text>
                                                </div>
                                                <div className="col-3">
                                                    <input type="text" name={`C${key}`}  onChange={(e)=>this.calcule(key,e)} placeholder="Entrer le cout d'une seul laivraison" />
                                                </div>
                                                <div className="col-3">
                                                    <Text><b>Totale: { item.totalelivreur } DA</b></Text><br />
                                                    <Text><b>Livreur: { item.nbc * this.state.values[key] } DA</b></Text><br />
                                                    <Text><b style={{color:"green"}}>Bénifice Net: { item.totalelivreur-(item.nbc * this.state.values[key]) } DA</b></Text><br />
                                                    {this.state.values[key] ?
                                                    <Button style={{width:"100%",backgroundColor:"green",color:"white"}} onClick={(e)=>this.Submit(key,e,item.nbc * this.state.values[key],item.totalelivreur-(item.nbc * this.state.values[key]),item)}><b>Enregister</b></Button>
                                                        :(
                                                            <Button disabled="true" style={{width:"100%",backgroundColor:"red",color:"white"}} onClick={(e)=>this.Submit(key,e,item.nbc * this.state.values[key],item.totalelivreur-(item.nbc * this.state.values[key]),item)}><b>Enregister</b></Button>
