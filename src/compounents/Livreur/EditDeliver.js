...
import '../../App.css';
import {DollarCircleOutlined,PhoneOutlined,StarOutlined,MoneyCollectOutlined, MenuOutlined, LogoutOutlined, ToolOutlined, ShoppingCartOutlined, UsergroupAddOutlined, MoreOutlined, ShoppingOutlined, MenuUnfoldOutlined, DashboardOutlined, EditOutlined, ChromeOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
const openNotification = () => {
    ...
    notification.open({
        message: 'Bon travail',
        description:
            "Le Livreure a été Modifier.",
        btn,
        key,
        onClose: close,
    });
};
class EditDeliver extends Component {
    constructor() {
        super()
        this.state = {
            first_name: "",
                ...
                if (!findresponse.status===200 || findresponse.msg==='Not enough segments') {
                    ...
                    if(!findresponse.user){
                        ...
                        if(findresponse.user.role==="Livreur"){
                            ...
                            fetch(`http://151.80.56.201:5000/Dashboard/EditDelivrer/${this.state.id}`, {
                                method: 'get', headers: {
                                    Accept: 'application/json',
                                    'Content-Type': 'application/json',
    ...
    onSubmit(e) {
        ...
        axios({
            url: `http://151.80.56.201:5000/Dashboard/EditDelivrer/${this.state.id}`,
            method: 'POST',
            data: formData,
            headers: {
