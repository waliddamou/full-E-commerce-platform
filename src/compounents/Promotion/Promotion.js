...
import '../../App.css';
import { MoneyCollectOutlined, PhoneOutlined, StarOutlined, MenuOutlined, LogoutOutlined, ToolOutlined, ShoppingCartOutlined, UsergroupAddOutlined, MoreOutlined, ShoppingOutlined, MenuUnfoldOutlined, DashboardOutlined, EditOutlined, ChromeOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
class Promotion extends Component {
                ...
                if (!findresponse.status === 200 || findresponse.msg === 'Not enough segments') {
                    ...
                    if (!findresponse.user) {
                        ...
                        if (findresponse.user.role === "Livreur") {
                            ...
                            let result = []
                            fetch('http://151.80.56.201:5000/Dashboard/AddPromotion', {
                                method: 'get', headers: {
                                    Accept: 'application/json',
                                    'Content-Type': 'application/json',
                                    'Authorization': "Bearer " + token,
                                ...
                                .then((findresponse) => {
                                    this.setState({
                                        products: findresponse.products.map((row, i) =>
                                            result.push([row.id, row.Name, row.Type, row.Subcategory, row.Mother, <Button onClick={() => this.add(row)} icon={<PlusCircleOutlined />}></Button>]))
                                    })
                                    this.setState({
                                        products: result
                                    })
    ...
    Submit(e) {
        ...
        axios({
            url: "http://151.80.56.201:5000/Dashboard/AddPromotion",
            method: 'POST',
            data: formData,
            headers: {
                'Content-Type': 'application/json',
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        ...
                        <div className="site-layout-background center" style={{ padding: 24, minHeight: 360 }}>
                            <div className="input-field">
                                <Text style={{ color: "#ad3216", fontWeight: "bold" }}>Produit : {this.state.oneproduct.Name}</Text>&nbsp;&nbsp;&nbsp;
                                <Text style={{ color: "#ad3216", fontWeight: "bold" }}>Prix de Vente : {this.state.oneproduct.SellPrice}</Text>
                            </div>
                            <div className="input-field">
                            ...
                            <div className="input-field">
                                ...
                                <Button onClick={(e) => this.Submit(e)} style={{ backgroundColor: "#198ffe", width: "100%", color: "white", fontWeight: "bold" }}>Lancer la Promotion</Button>
                            </div>
