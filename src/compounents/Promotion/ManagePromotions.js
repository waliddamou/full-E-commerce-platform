...
import '../../App.css';
import { DeleteOutlined, MenuOutlined, LogoutOutlined, PhoneOutlined,StarOutlined,MoneyCollectOutlined, ToolOutlined, ShoppingCartOutlined, UsergroupAddOutlined, MoreOutlined, ShoppingOutlined, MenuUnfoldOutlined, DashboardOutlined, EditOutlined, ChromeOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
class ManagePromotions extends Component {
            ...
            .then((findresponse) => {
                if (!findresponse.status === 200 || findresponse.msg === 'Not enough segments') {
                    ...
                    if (!findresponse.user) {
                        ...
                        if(findresponse.user.role === "Admin"){
                           
                                const result = []
                                fetch('http://151.80.56.201:5000/Dashboard/ManagePromotions', {
                                    method: 'get', headers: {
                                        Accept: 'application/json',
                                        'Content-Type': 'application/json',
                                        'Authorization': "Bearer " + token,
                                    ...
                                    .then((Response) => Response.json())
                                    .then((findresponse) => {
                                        findresponse.promoted.map((row, i) =>
                                            result.push([row.id, row.Name, row.Pourcentage, row.Prix, <><Link to={`/EditPromo/${row.id}`} ><EditOutlined /></Link> <Button onClick={() => this.DeletePromotion(row.id)} icon={<DeleteOutlined />}></Button>
                                            </>]))
                                        this.setState({
                                            Promoted: result
                                        })
    ...
    DeletePromotion = (id) => {
        ...
        axios({
            url: `http://151.80.56.201:5000/Dashboard/ManagePromotions`,
            method: 'POST',
            data: formData,
            headers: {
                'Content-Type': 'application/json',
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        ...
                        <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
                            <MUIDataTable
                                ...
                                columns={["ID", "Nom", "Pourcentage", "Prix Promo", "Actions"]}
                                data={this.state.Promoted}
                                options={{
                                    filterType: "checkbox",
                                    print: true
                                }
