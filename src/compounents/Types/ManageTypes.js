...
class ManageTypes extends Component {
    ...
    componentDidMount() {
        ...
        .then((findresponse)=>
        {   this.setState({
            Categories : findresponse.types.map((row, i) =>
            result.push([row.id, row.Name,row.Subcategory,row.Mother,<><Button href={"/EditType/"+row.id} icon={<EditOutlined />}></Button> <Button href={"/DeleteType/"+row.id} icon={<DeleteOutlined />}></Button></>])) })   
        } )
            ...
            <Layout style={{ minHeight: '100vh' }}>
                ...
                <Layout className="site-layout">
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        ...
                        <MUIDataTable
                        title={"Types"}
                        columns={["ID", "NOM","Sous Catégorie", "Catégorie Mére","Actions"]}
