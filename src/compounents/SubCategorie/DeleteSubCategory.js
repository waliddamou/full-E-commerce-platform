...
import { Layout, Menu, Divider, Typography, Button, notification, Breadcrumb } from 'antd';
import '../../App.css';
import {PhoneOutlined,MoneyCollectOutlined,StarOutlined, DollarCircleOutlined,DeleteOutlined, MenuOutlined, LogoutOutlined, ToolOutlined, ShoppingCartOutlined, UsergroupAddOutlined, MoreOutlined, ShoppingOutlined, MenuUnfoldOutlined, DashboardOutlined, EditOutlined, ChromeOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
const openNotification = () => {
    ...
    notification.open({
        message: 'Bon travail',
        description:
            'La Sous Catégorie a été Supperimer.',
        btn,
        key,
        onClose: close,
    });
...
class DeleteSubCategory extends Component {
    ...
    onSubmit(e) {
        ...
        axios({
            url: "http://151.80.56.201:5000/Dashboard/DeleteSubCategories/" + id,
            method: 'POST',
            data: formData,
            headers: {
                ...
                this.props.history.push('/ManageSubCategories')
    ...
    componentDidMount() {
        ...
        fetch('http://151.80.56.201:5000/Dashboard/EditSubCategories/'+id, {
            method: 'get', headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
                'Authorization': "Bearer " + localStorage.getItem('usertoken'),
        ...
        { 
            this.setState({
                name : findresponse.Categories_list[0].Name
                ...
                <Layout className="site-layout">
                    ...
                    <Content style={{ margin: '0 16px' }}>
                        <Breadcrumb style={{ margin: '16px 0' }}>
                            ...
                            <Breadcrumb.Item>Supprimer une Sous Catégorie</Breadcrumb.Item>
                        </Breadcrumb>
                        <div className="site-layout-background" style={{ padding: 24, minHeight: 360 }}>
                            <div className="col s12 m6">
                                <span>
                                    <Text style={{ color: "black" }}>
                                    êtes-vous sûr de supprimer cette Categorie ?
                             </Text>
                                </span>
                                <form noValidate onSubmit={this.onSubmit}>
                                    <div className="input-field">
                                        <input disabled type="text" name="name" value={this.state.name} onChange={this.onChange} id="name" className="validate" />
                                        </div>
                                    
                                        <button onClick={openNotification} type="submit" className="btn red align-center" style={{ background: "#ad3216" }}>{<DeleteOutlined spin />} Supprimer</button>
